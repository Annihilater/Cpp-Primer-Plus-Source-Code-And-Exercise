...
int main() {
    ...
    if (customers > 0) {
        ...
        std::cout << (double) sum_line / (double) cyclelimit << std::endl;
        std::cout << "average wait time: ";
        std::cout << (double) line_wait / (double) served << " minutes\n";
    } else {
...
bool newcustomer(double x) {
    return ((double) random() * x / RAND_MAX < 1);
}
//-------------------------------------------------------------------------------------------------
